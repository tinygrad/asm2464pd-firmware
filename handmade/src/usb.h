...
#define USB_STR_PRODUCT         "custom " GIT_VERSION
...
static void usb_serdes_tune_lane(uint16_t base) {
    XDATA_REG8(base + 0x02) = 0xAA; // b[4:0] correlates with TX drive strength
    XDATA_REG8(base + 0x03) = 0x81;
    XDATA_REG8(base + 0x04) = 0x45; XDATA_REG8(base + 0x05) = 0x6F;
    XDATA_REG8(base + 0x06) = 0x77; XDATA_REG8(base + 0x07) = 0x6F;
    ...
    rmw(base + 0x43, 0xC3, 0x3C); rmw(base + 0x45, 0xF0, 0x0B);
    rmw(base + 0x46, 0xF0, 0x0D); rmw(base + 0x49, 0x80, 0x41);
    rmw(base + 0x4A, 0xFE, 0x00); rmw(base + 0x4C, 0xF1, 0x00);
