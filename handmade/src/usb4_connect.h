...
static void bank0_8a89(uint8_t mode) {
  ...
  if (link_mode < 3) {
    ...
    if (PR(0x0A9D) == 0x02) {
      if (config & 0x02) {
        PR(0x0A9D) = (REG_LINK_STATUS_E716 & 0xFC);
        U4C_BCFE(0xCC3E);
    ...
    PR(0x0A9F) |= (REG_LINK_WIDTH_E710 & 0xE0);
    PR(0x0A9E) = (REG_CPU_MODE_NEXT & 0x1F) | (uint8_t)(PR(0x0A9E) << 5);
    REG_CPU_CTRL_CA81 &= 0xFE;

    /* second mode dispatch selecting the descriptor arg */
    if (PR(0x0A9D) == 0x02) {
      if (config & 0x02) {
        PR(0x0A9D) = (REG_LINK_STATUS_E716 & 0xFC) | 0x03;
        PR(0x0A9E) = (REG_CPU_CTRL_CC3E & 0xFD);
        U4C_BCEB(0xCA81);
        u4c_bd14();
      }
...
static void bank0_c9a8(uint8_t mode) {
  ...
  if (PR(0x09FA) & 0x04) {
    sb_tunnel_up_pending = 1;
    if ((PR(0x0AF1) & 0x01) &&
        (PR(0x07E8) != 0 || PR(0x07EB) != 0)) {
      REG_PHY_CFG_C6A8 &= 0xFE;
      bank0_8a89(PR(0x0A7D));
    }
    PR(0x07E8) = 0x00;
    PR(0x0B2F) = 0x01;
    return;
