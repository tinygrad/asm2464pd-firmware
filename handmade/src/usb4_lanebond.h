...
static void u4lb_ce23(uint8_t ctrl_bits, uint8_t set_lanes) {
  ...
  u4lb_e00c();
  u4lb_e7f8((uint8_t)((P12_RD(0x35) & 0xC0) | r6));
...
static void u4lb_b031_transport_reinit(uint8_t skip_lane) {
  ...
  u4lb_b34_lanemask = 0; u4lb_b35 = 0; u4lb_b36 = 0; u4lb_b37 = 0;
  u4c_e5b0();
  u4c_e8d6_seed_workbuf();
  sb_rom_descriptor_load();
  u4lb_c17f();
  u4c_d4c8_edge_engine();
  u4c_e4d2_edge();
  u4c_cbf8_commit();
  u4c_dcb4_transport_reg_reinit();
  if (!skip_lane) {
    u4lb_e06b(0);
  }
  u4lb_e74e();
...
static void u4lb_e1cb_e2b9(uint8_t is_e1cb) {
  ...
  SBTX_WR(1, (uint8_t)(sb_tx_byte1 | ((aab & 1) << 7)));
  if (aab != 0)
    form = (uint8_t)(((sb_tx_byte0 + 8) & 0xFF) | (SB_RD(0x0C) & 0x80));
  else
    form = (uint8_t)((SB_RD(0x0C) & 0x80) | 0x08);
  SB_WR(0x0C, form);
  if (is_e1cb)
    SB_WR(0x15, (uint8_t)((sb_tx_cmd << 1) | 0x41));
  else
