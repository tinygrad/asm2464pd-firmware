#!/usr/bin/env python3
"""Flash user firmware through the bootstub's vendor EP0 protocol."""
...
def require(condition, message, exc=RuntimeError):
  if not condition:
    raise exc(message)
...
def control(h, request, value=0, index=0, data=b"", read=0, timeout=TIMEOUT_MS):
  ...
  require(ret == length, f"short control transfer: {ret}/{length}")
...
def get_info(h):
  ...
  valid = (magic == b"A24D" and abi == BOOTSTUB_ABI_VERSION and page == PAGE and sector == SECTOR_SIZE
           and start == USERFW_FLASH_OFFSET and end == USERFW_IMAGE_END)
  require(valid, f"incompatible DFU: {info}")
  return page


def set_addr(h, addr):
  require(0 <= addr < 0x1000000, f"invalid flash address: 0x{addr:X}", ValueError)
  control(h, 0xB1, value=addr & 0xFFFF, index=addr >> 16)


def write_at(h, page, address, data):
  for off in range(0, len(data), page):
    set_addr(h, address + off)
    control(h, 0xB2, data=data[off:off + page])


def read_at(h, address, length):
  set_addr(h, address)
  return b"".join(control(h, 0xB3, read=min(PAGE, length - off)) for off in range(0, length, PAGE))


def validate_image(image):
  require(USERFW_HEADER_SIZE < len(image) <= USERFW_IMAGE_END - USERFW_FLASH_OFFSET,
          "invalid user firmware image size", ValueError)
  magic, abi, body_len, expected_crc = struct.unpack("<4sBII", image[:13])
  require(magic == b"A24F", "not a packed user firmware image", ValueError)
  require(abi == BOOTSTUB_ABI_VERSION, f"unsupported bootstub ABI: {abi}", ValueError)
  require(body_len != 0 and body_len == len(image) - USERFW_HEADER_SIZE,
          "user firmware length mismatch", ValueError)
  actual_crc = zlib.crc32(image[:9] + image[USERFW_HEADER_SIZE:]) & 0xFFFFFFFF
  require(actual_crc == expected_crc, "user firmware CRC mismatch", ValueError)


def router_regs():
  routers = [p.parent.name for p in TB_SYSFS.glob("*/vendor") if int(p.read_text(), 16) == USERFW_ID[0]]
  require(len(routers) <= 1, f"{len(routers)} ADD1 USB4 routers present")
...
def enter_dfu():
  ...
  if regs := router_regs():
    ...
    return
  raise RuntimeError("no application or bootstub found")
...
if __name__ == "__main__":
  ...
  require(len(usb_nodes(USERFW_ID[0])) <= 1, "multiple ADD1 USB devices present")
  if args.command == "flash":
    require(args.image, "flash requires an image", ValueError)
    ...
    require(erase_len <= USERFW_ERASE_END - USERFW_FLASH_OFFSET, "erase exceeds user firmware partition")
    ...
    # Keep magic erased until every other byte is programmed and verified. A
    # reset at any earlier point therefore returns to DFU instead of booting.
    print(f"  writing and verifying {len(image)} bytes")
    write_at(h, page, USERFW_FLASH_OFFSET + USERFW_HEADER_SIZE, image[USERFW_HEADER_SIZE:])
    write_at(h, page, USERFW_FLASH_OFFSET + 4, image[4:USERFW_HEADER_SIZE])
    require(read_at(h, USERFW_FLASH_OFFSET + USERFW_HEADER_SIZE,
                    len(image) - USERFW_HEADER_SIZE) == image[USERFW_HEADER_SIZE:], "body verification failed")
    require(read_at(h, USERFW_FLASH_OFFSET + 4,
                    USERFW_HEADER_SIZE - 4) == image[4:USERFW_HEADER_SIZE], "header verification failed")
    write_at(h, page, USERFW_FLASH_OFFSET, image[:4])
    require(read_at(h, USERFW_FLASH_OFFSET, 4) == image[:4], "commit verification failed")
    print("  verify OK; image committed")
