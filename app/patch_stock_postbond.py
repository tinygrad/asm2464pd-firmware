...
EA_BUDGET_N = 0x60         # log the first 0x60 (96) eaac calls in counter order
...
def build_ea_hook():
    ...
    # --- POST-BOND ARM GATE: only log eaac calls AFTER the lane bond completes
    # (SB[0xA0]==0x02). The pre-bond CL/route walk (desc 0D) was already captured by
    # the non-arm variant and would exhaust the budget before the bond. By skipping
    # until SB[0xA0]==0x02 the budget covers the POST-BOND window where the host
    # posts the cm8 (desc_type 0x08) lane-management commands. We re-check SB[0xA0]
    # each call (harmless: after bond it stays 0x02). Read SB[0xA0] via DPX=1 paged.
    #   mov DPX,#1 ; movx a,SB[A0] ; mov DPX,#0
    #   cjne a,#0x02,not_bonded ; <fall through if ==0x02> sjmp ARMED
    #   not_bonded: ljmp SKIP
    #   ARMED:
    code += bytes([0x75, 0x93, SB_DPX])            # mov DPX,#1
    code += mov_dptr(0x2800 + 0xA0) + b"\xe0"      # movx a,@dptr (SB[0xA0])
    code += bytes([0x75, 0x93, 0x00])              # mov DPX,#0 (restore)
    cjne_pos = len(code)
    code += b"\xb4\x02\x00"                        # cjne a,#0x02,not_bonded (rel filled below)
    code += b"\x80\x00"                            # sjmp ARMED (==0x02; rel filled below)
    sjmp_pos = cjne_pos + 3
    not_bonded = len(code)
    code += b"\x02\x00\x00"                        # ljmp SKIP (not bonded yet; addr filled at end)
    armed = len(code)
    code[cjne_pos + 2] = (not_bonded - (cjne_pos + 3)) & 0xFF  # cjne rel -> not_bonded
    code[sjmp_pos + 1] = (armed - (sjmp_pos + 2)) & 0xFF       # sjmp rel -> ARMED
    ljmp_arm_skip = not_bonded                     # this ljmp also needs SKIP addr
    ...
    code[ljmp_skip + 1] = (skip_abs >> 8) & 0xFF
    code[ljmp_skip + 2] = skip_abs & 0xFF
    # the post-bond arm-gate not_bonded ljmp also targets SKIP
