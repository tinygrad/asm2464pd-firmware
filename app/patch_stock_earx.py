...
def build_ea_field_block():
    ...
    # "|2a=" host descriptor source plane 0x2a00[0..7] (DPX=1 paged) -- byte0=desc_type (port 0)
    code += puts_code(SEP_2A)
    code += dump_block(0x2A00, 0x08, SB_DPX)
    # "|2b=" host descriptor source plane 0x2b00[0..7] (DPX=1 paged) -- the rx plane when port 1
    code += puts_code(SEP_2B)
    code += dump_block(0x2B00, 0x08, SB_DPX)
    # "|tx=" device reply plane 0x2900[0..7] (DPX=1 paged) -- the af38 TX (prev descriptor's reply)
    ...
    # "|6f1=" active port 0x06F1 (DPX=0) -- tells which plane (2a vs 2b) was the rx plane
    code += puts_code(SEP_6F1)
    code += puthex_xdata(0x06F1)
    # close
    code += mov_dptr(UART_TX) + b"\x74\x5d\xf0"     # ']'
    return bytes(code)
...
def apply_patch(body):
    ...
    write_cave(body, SEP_6F1, b"|6f1=\x00", "6f1")
