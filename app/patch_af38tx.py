...
def build_af_hook():
    ...
    # --- change gate on SBTX[5] = paged XDATA 0x2905 (DPX=1). Keep it in SFR B
    # (0xF0, register-bank-INDEPENDENT) so the XRL compare is robust regardless of
    # PSW RS1:RS0. SB read needs DPX=1; XDATA scratch reads need DPX=0.
    code += bytes([0x75, 0x93, SB_DPX])            # mov DPX,#1
    code += mov_dptr(0x2900 + 0x05) + b"\xe0"      # movx a,@dptr  (SBTX[5])
    code += b"\xf5\xf0"                            # mov B,a       (B = SBTX5, bank-safe)
    code += bytes([0x75, 0x93, 0x00])              # mov DPX,#0
    # if SEEN==0 -> force-emit (first line). Else compare SBTX5 (B) vs AF_LAST.
    # Use the LJMP-skip pattern (template-consistent / size-robust).
    code += mov_dptr(AF_SEEN) + b"\xe0"            # movx a,@dptr (SEEN)
    jz_emit = len(code)
    code += b"\x60\x00"                            # jz EMIT  (SEEN==0 -> first time)
    code += mov_dptr(AF_LAST) + b"\xe0"            # movx a,@dptr (LAST) -> A
    code += b"\x65\xf0"                            # xrl A,B   (A ^= SBTX5)
    jnz_emit = len(code)
    code += b"\x70\x00"                            # jnz EMIT  (SBTX5 changed -> over LJMP)
    ljmp_skip = len(code)
    ...
    # --- EMIT: update last-seen (AF_LAST = B = SBTX5), mark seen, print the line.
    code += mov_dptr(AF_LAST) + b"\xe5\xf0\xf0"    # mov a,B ; movx @dptr,a (LAST=SBTX5)
