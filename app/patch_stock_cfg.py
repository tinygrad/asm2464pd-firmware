...
CAVE_B8 = 0x6D00           # b8db (CDR/PLL validate) hook -- per-lane lock during training

# b8db entry: `7b02 7a00` = MOV R3,#2 ; MOV R2,#0 (2 instructions, 4 bytes). Displace
# 4 bytes -> LCALL(3) + 1 NOP. Samples per-lane lock C2D0(laneA)/C350(laneB) at the
# CDR/PLL validate entry -- the SAME point handmade's [c2@b8db] samples. Low-frequency
# (called a few times per state-4 training, NOT the hot walker).
H_B8_OFF = bank1_off(0xB8DB)
H_B8_OLD = bytes.fromhex("7b027a00")
STR_B8 = 0x70D0            # "\r\n[bk A="
LBL_BKB = 0x70E0           # " B="
...
# ---- b8db hook (per-lane lock during CDR/PLL validate) ----------------------
def build_b8_hook():
    """push preserve; DPX=0; print [bk A=<C2D0> B=<C350>]; pop; replay; ret.
    NOT change-gated (called only a few times per training) so every validate
    pass is visible -- the fine-grained per-lane lock timeline. Compares directly
    to handmade's in-tree [c2@b8db]."""
    code = bytearray()
    for d in PRESERVE:
        code += bytes([0xC0, d])                   # push direct
    code += bytes([0x75, 0x93, 0x00])              # mov DPX,#0
    code += puts_code(STR_B8)                      # "\r\n[bk A="
    code += puthex_xdata(0xC2D0)                   # laneA lock (bit6 PLL, bit5 CDR)
    code += puts_code(LBL_BKB)                     # " B="
    code += puthex_xdata(0xC350)                   # laneB lock
    code += mov_dptr(UART_TX) + b"\x74\x5d\xf0"    # ']'
    for d in reversed(PRESERVE):
        code += bytes([0xD0, d])                   # pop direct
    code += H_B8_OLD                               # replay MOV R3,#2 ; MOV R2,#0
    code += b"\x22"                                # ret
    return bytes(code)
...
def apply_patch(body):
    ...
    write_cave(body, STR_B8, b"\r\n[bk A=\x00", "bk str")
    write_cave(body, LBL_BKB, b" B=\x00", "bkB")
    ...
    b8_hook = build_b8_hook()
    write_cave(body, CAVE_B8, b8_hook, "b8 hook")
    patch_site(body, H_B8_OFF, H_B8_OLD, CAVE_B8, "b8db")

    return [
        ("a66", H_A66_OFF, CAVE_A66, len(a66_hook), H_A66_OLD),
        ("ea", H_EAAC_OFF, CAVE_EA, len(ea_hook), H_EAAC_OLD),
        ("b8", H_B8_OFF, CAVE_B8, len(b8_hook), H_B8_OLD),
    ]
