...
def build_ea_hook():
    """push preserve; DPX=0; BUDGET-GATE (af38seq variant): log EVERY eaac call in
    counter order until a decrementing budget is exhausted; pop; replay displaced
    bytes; ret.

    This replaces patch_stock_cfg.py's change-gate (which gates on XDATA[0x077A] and
    therefore MISSES every eaac call that re-posts the same 077A). The budget gate
    emits the FULL ordered desc-type sequence (byte[0] of 0x2a00) the host posts.

    Cells reused: EA_SEEN (0x0B59) = "budget initialized" flag; EA_LAST (0x0B58) =
    remaining budget. On the first call (SEEN==0) we seed EA_LAST=BUDGET and SEEN=1.
    Each subsequent call: if EA_LAST==0 -> LJMP SKIP (budget spent); else decrement
    EA_LAST and EMIT.

    NOTE: eaac OVERWRITES 0x077A during its copy, so the line still reads 0x2a00 /
    0x077A BEFORE the displaced MOV DPTR,#0x775 runs."""
    BUDGET = 0x50                                  # ~80 eaac calls logged in order
    code = bytearray()
    for d in PRESERVE:
        code += bytes([0xC0, d])                   # push direct
    code += bytes([0x75, 0x93, 0x00])              # mov DPX,#0

    # --- budget gate (DPX=0). First call: seed budget. Then: spend-or-skip.
    code += mov_dptr(EA_SEEN) + b"\xe0"            # movx a,@dptr (SEEN/init flag)
    jnz_have = len(code)
    code += b"\x70\x00"                            # jnz HAVE  (already initialized)
    # --- first call: SEEN=1, EA_LAST=BUDGET, fall through to EMIT
    code += mov_dptr(EA_SEEN) + bytes([0x74, 0x01, 0xF0])   # SEEN=1
    code += mov_dptr(EA_LAST) + bytes([0x74, BUDGET, 0xF0]) # EA_LAST=BUDGET
    sjmp_emit = len(code)
    code += b"\x80\x00"                            # sjmp EMIT
    have = len(code)
    code[jnz_have + 1] = (have - (jnz_have + 2)) & 0xFF
    # --- subsequent calls: if EA_LAST==0 -> SKIP; else dec EA_LAST -> EMIT
    code += mov_dptr(EA_LAST) + b"\xe0"            # movx a,@dptr (budget) -> A
    jnz_spend = len(code)
    ...
    code += b"\x14"                                # dec a
    code += mov_dptr(EA_LAST) + b"\xf0"           # movx @dptr,a (EA_LAST--)

    emit = len(code)
    code[sjmp_emit + 1] = (emit - (sjmp_emit + 2)) & 0xFF

    # --- EMIT: print the line. (No 077A capture into B needed anymore.)
