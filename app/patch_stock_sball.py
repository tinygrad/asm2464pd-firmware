...
SEP_2A = 0x70C0            # "|2a="
SEP_6F0 = 0x70C8           # "|6f0="
SEP_SBALL = 0x70D0         # "|SBALL="   (256-reg dump)
SEP_XALL = 0x70D8          # "|XALL="    (XDATA block)
SEP_6EC = 0x70E0           # "|6EC="     (XDATA tail singletons)
SEP_FULL = 0x70E8          # "|FULL="    (comprehensive 0x0700-0x0BFF dump)

CAVE_A66 = 0x6600          # a066 hook (the larger of the two; grown for the 256-reg dump)
...
EA_LAST = 0x0B58           # last-seen XDATA[0x077A]
EA_SEEN = 0x0B59           # eaac seen flag
...
def build_a66_field_block():
    ...
    # PHY-INPUT probe (this session): per-lane PLL/CDR lock (DPX=0) + width-event SOURCE (DPX=1 P1).
    # plA=C2D0/C297 laneA, plB=C350/C317 laneB (bit6=PLL .5=full / bit4=CDR). 1407/1603 = the C80A.4
    # secondary-adapter event source. E716 = width-status. rate C294/C314.
    code += puthex_xdata(0xC2D0)
    code += puthex_xdata(0xC297)
    code += puthex_xdata(0xC350)
    code += puthex_xdata(0xC317)
    code += puthex_xdata(0xC294)
    code += puthex_xdata(0xC314)
    code += puthex_xdata(0xE716)
    code += puthex_sb(0x1407)
    code += puthex_sb(0x1603)
    # *** COMPREHENSIVE 256-SB-REGISTER DUMP (the decisive frame-level diff). ***
    # "|SBALL=" then ALL 256 SB registers SB[0x00..0xFF] (DPX=1 paged 0x2800+off). The dump_block
    # count field is one byte; 0x100 wraps to 0, so dump in two 0x80 halves. Matches handmade
    # main.c [SBALL=...] (512 hex chars).
    code += puts_code(SEP_SBALL)
    code += dump_block(0x2800, 0x80, 0x01)
    code += dump_block(0x2880, 0x80, 0x01)
    # "|XALL=" host-commit XDATA block 0x0777..0x081F (DPX=0), 0xA9 bytes -> matches handmade
    # [XALL 0777=...] (the 0x0777-0x081F lane/route/advertise block).
    code += puts_code(SEP_XALL)
    code += dump_block(0x0777, 0x80, 0x00)
    code += dump_block(0x07F7, 0x29, 0x00)
    # *** COMPREHENSIVE FULL-RANGE XDATA DUMP 0x0700-0x0BFF (DPX=0), the 17th-agent's missed-seed sweep.
    # 0x500 bytes in ten 0x80-byte chunks. Matches handmade main.c [FULL0700=...] (2560 hex chars).
    code += puts_code(SEP_FULL)
    for base in range(0x0600, 0x0C00, 0x80):
        code += dump_block(base, 0x80, 0x00)
    # plus the 0x09Fx cap block 0x09F0-0x09FF explicitly (inside 0x0900-0x097F? no -> 0x0980 chunk
    # covers 0x0980-0x09FF, so 0x09Fx IS in the FULL dump above; kept here for clarity, no extra dump).
    # "|6EC=" tail XDATA singletons: 6EC/6ED 6F0/6F1 718/719 9F9/9FA (DPX=0)
    code += puts_code(SEP_6EC)
    code += puthex_xdata(0x06EC); code += puthex_xdata(0x06ED)
    code += puthex_xdata(0x06F0); code += puthex_xdata(0x06F1)
    code += puthex_xdata(0x0718); code += puthex_xdata(0x0719)
    code += puthex_xdata(0x09F9); code += puthex_xdata(0x09FA)
    code += puthex_xdata(0x09F4); code += puthex_xdata(0x07BE)  # dp-alt mode + fallback (09FA root)
    # close
...
def build_ea_hook():
    """push preserve; DPX=0; CHANGE-GATE on XDATA[0x077A]; if changed emit the line
    + update last-seen; pop; replay displaced bytes; ret.

    NOTE: eaac OVERWRITES 0x077A during its copy, so we read 0x077A BEFORE the
    displaced MOV DPTR,#0x775 runs -- i.e. the gate/line capture the host-posted
    value from the PREVIOUS pass's mirror, which is exactly the watched evolution."""
    code = bytearray()
    for d in PRESERVE:
        code += bytes([0xC0, d])                   # push direct
    code += bytes([0x75, 0x93, 0x00])              # mov DPX,#0

    # --- change gate on XDATA[0x077A] (DPX=0). Keep it in SFR B (bank-safe).
    code += mov_dptr(0x077A) + b"\xe0"             # movx a,@dptr  (077A)
    code += b"\xf5\xf0"                            # mov B,a       (B = 077A)
    code += mov_dptr(EA_SEEN) + b"\xe0"            # movx a,@dptr (SEEN)
    jz_emit = len(code)
    code += b"\x60\x00"                            # jz EMIT  (SEEN==0 -> first time)
    code += mov_dptr(EA_LAST) + b"\xe0"            # movx a,@dptr (LAST) -> A
    code += b"\x65\xf0"                            # xrl A,B   (A ^= 077A)
    jnz_emit = len(code)
    code += b"\x70\x00"                            # jnz EMIT  (077A changed -> over LJMP)
    ljmp_skip = len(code)
    code += b"\x02\x00\x00"                        # ljmp SKIP ; placeholder addr
    ...
    # --- EMIT: update last-seen (EA_LAST = B = 077A), mark seen, print the line.
    code += mov_dptr(EA_LAST) + b"\xe5\xf0\xf0"    # mov a,B ; movx @dptr,a (LAST=077A)
...
def apply_patch(body):
    ...
    write_cave(body, SEP_SBALL, b"|SBALL=\x00", "SBALL")
    write_cave(body, SEP_XALL, b"|XALL=\x00", "XALL")
    write_cave(body, SEP_6EC, b"|6EC=\x00", "6EC")
    write_cave(body, SEP_FULL, b"|FULL=\x00", "FULL")
