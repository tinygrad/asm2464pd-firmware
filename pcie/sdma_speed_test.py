...
Strategy: batch ring packets with pipeline depth 2 (wait for batch N-1's
signal before submitting batch N+1), then VERIFY every byte moved.
...
negotiated link can be confirmed by the throughput ratio even with host-side
...
def sdma_run(sram_va, dst_va, dst_span, total, label):
  ...
  while done < reps:
    ...
    q = AMDCopyQueue(amd).wait(amd.timeline_signal, amd.timeline_value - 1)
    for i in range(n):
      k = done + i
    ...
    q = q.signal(amd.timeline_signal, amd.next_timeline())
    q.submit(amd)
    done += n
    # pipeline depth 2: only wait for the batch before last
    if prev_sig is not None:
      amd.timeline_signal.wait(prev_sig, timeout=120000)
    prev_sig = amd.timeline_value - 1
  amd.timeline_signal.wait(amd.timeline_value - 1, timeout=120000)
  dt = time.perf_counter() - t0
  print(f"  {label}: {total >> 20}MB in {dt*1e3:.1f}ms = {total/dt/1e6:.0f} MB/s ({total/dt/1e9:.2f} GB/s)")
  return total / dt / 1e9
