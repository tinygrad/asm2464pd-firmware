...
def create_hardware_hooks(memory: 'Memory', hw: HardwareState, proxy: 'UARTProxy' = None, proxy_mask: list = None):
    ...
    # ============================================
    def make_proxy_read_hook(proxy_ref, hw_ref, mem_ref):
        ...
        def hook(addr):
            ...
            if proxy_ref.debug >= 2:
                ...
                cyc = hw_ref.cycles
                dpx = mem_ref.sfr[0x93 - 0x80]
                dpx_tag = f" DPX={dpx}" if dpx else ""
                reg_name = get_register_name(addr)
                if reg_name:
                    print(f"[{cyc:8d}] PC=0x{pc:04X} Read  0x{addr:04X} = 0x{value:02X}  {reg_name}{dpx_tag}")
                else:
                    print(f"[{cyc:8d}] PC=0x{pc:04X} Read  0x{addr:04X} = 0x{value:02X}{dpx_tag}")
            return value
        return hook

    def make_proxy_write_hook(proxy_ref, hw_ref, mem_ref):
        ...
        def hook(addr, value):
            ...
            if proxy_ref.debug >= 2:
                ...
                reg_name = get_register_name(addr)
                if reg_name:
                    print(f"[{cyc:8d}] PC=0x{pc:04X} Write 0x{addr:04X} = 0x{value:02X}  {reg_name}{dpx_tag}")
                else:
                    print(f"[{cyc:8d}] PC=0x{pc:04X} Write 0x{addr:04X} = 0x{value:02X}{dpx_tag}")
        return hook
    ...
    if proxy is not None:
        ...
        proxy_read_hook = make_proxy_read_hook(proxy, hw, memory)
        proxy_write_hook = make_proxy_write_hook(proxy, hw, memory)
